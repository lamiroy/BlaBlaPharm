...
def algo():
    if request.method=='POST':
        ...
        for i in range(mint):
            ...
            for k in range(1,p):
                for l in range(n):
                    sommeProb-=2*x[i][l*p+k]*distance2[l][n+i] # cas ou le pharmacien n'a pas de creneau juste avant ou apres on met allé retour
        ...
        stat1=nbcrefin/nbcrecan*100
        stat2=nbcrefin/nbcrepha*100
        print(resul)
        print("mo")
        print(mo)
        print("c")
        ...
        print("kilomètres")
        return render_template("affalgo.html",resultat=resul,nbcandidatt=nbcandidat,mo=mo,c=c,time=end-start,stat1=stat1,stat2=stat2,dist=distfinaleparcouru)
